...
public class GerritDownloaderTester {
	...
	public static void main(String[] args) throws IOException {
		...
		String outpuDir = "target/java-classes/";
		int change_ID = 1411;
		int revision = 1;
		ArrayList<String> files = downloader.getChangedFiles(url, change_ID, revision);
		
		for(String fileName : files) {
			String basicName = (fileName.lastIndexOf("/") == -1) ? fileName : fileName.substring(fileName.lastIndexOf("/") + 1);
			if(basicName.contains(".java")) {
				String base64String = downloader.getFileContent(url, change_ID, 1, fileName, true);
				byte[] baseFileContent = Base64.decodeBase64(base64String);
				File file = new File(outpuDir + change_ID + "_" + revision + "_old_" + basicName);
				BufferedOutputStream writer = new BufferedOutputStream(new FileOutputStream(file));
				writer.write(baseFileContent);
				writer.flush();
				writer.close();
				
				base64String = downloader.getFileContent(url, change_ID, 1, fileName, false);
				byte[] revFileContent = Base64.decodeBase64(base64String);
				file = new File(outpuDir + change_ID + "_" + revision + "_new_" + basicName);
